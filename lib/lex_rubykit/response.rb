module LexRubykit
  class Response
    ...
    require 'json'
    require 'lex_rubykit/response/dialog'

    attr_accessor :version, :session, :response_object, :session_attributes, :dialog_action,
                  :speech, :reprompt, :response, :card, :intents, :request, :should_end_session

    def initialize(request = nil)
      @session_attributes = Hash.new
      @request = request
      @intents = request.intent if request
      @dialog_action = DialogAction.new
    end
    ...
    def add_speech(speech_text, ssml = false)
      if ssml
        ...
        dialog_action.message_content_type = LexRubykit::DialogAction::MESSAGE_CONTENT_TYPE[:plain]
    ...
    # def say_response(speech, ssml = false)
    ...
    def build_response_object(session_end = true)
      @response = Hash.new
      @response[:dialogAction] = dialog_action.build_response_object(session_end)
      @response
    end
